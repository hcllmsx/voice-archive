...
(function () {
  ...
  L.en = {
    ...
    ui: {
      ...
      projectSettingsHint: 'Only the nickname and model name can be changed. Age group, language and purpose are fixed when the project is created — changing them would swap in a whole new set of guide sentences and your recordings would no longer line up.',
      ...
      allDone: 'All done!',
      allDoneBody: 'You\'ve preserved <b>{n}</b> clips<br><b>{total}</b> in total.',
      allDoneHint: 'You can still tap "Add a line" below to keep recording.',
      btnExport: 'Export',
      btnViewClips: 'View clips',
      hideAllLines: 'Hide all lines',
      showAllLines: 'Show all lines ({n})',
      customGroup: 'Extra',
      btnAddCustom: '＋ Add line',
      ...
      btnRemove: 'Remove',
      delCustomTitle: 'Remove this line?',
      delCustomBody: '“{text}” will be removed from the list and its recording deleted. This cannot be undone.',
      ...
      statRefHint: 'The ☆ star in each item: mark that clip as the reference audio — when you synthesize new speech later, its voice is used as the timbre reference. Tap again to unmark. Natural 3–10 second takes work best: GPT-SoVITS expects reference audio within 3–10 seconds, and anything longer can be trimmed with its reference-audio trimmer on the computer.',
      ...
      targetPathHint: 'Base folder where recordings live on the computer. If GPT-SoVITS is at D:\\GPT-SoVITS\\, enter that. The exported zip already contains a VoiceArchive-<model> folder — unzip it here and train. Prefer ASCII paths with no spaces.',
      exportPathNote: 'This training pack writes every dataset.list line against: {path}. The zip already contains a VoiceArchive-<model name> folder — just unzip it into the base folder. The base folder is a global setting (Settings → Computer target path) shared by all projects.',
      btnExportArchive: 'Export audio',
      ...
      badRefTip: 'Heads up: {n} starred reference clip(s) are outside 3–10 seconds. GPT-SoVITS expects reference audio within 3–10 seconds — if one runs long, trim it with the reference-audio trimmer on the computer.',
